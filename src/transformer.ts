...
/** Generates mangle names a b ... A B .... aa ... */
...
export interface tsMangleOptions {
  /**
   * Output directory for saving mangled .ts source files.
   * If not set, mangled .ts source files will not be saved.
   * Do not set this option for afterDeclarations transformer - the .ts files
   * would be incomplete.
   * (Default: unset)
   */
  mangledTsOutDir?: string;
  /**
   * Mangle names using this prefix + original name.
   * It can be usedull with uglify-js for example, which can do mangling
   * based on regexp for this prefix.
   * If not set or empty, the names will be generated.
   * (Default: unset)
   */
  manglePrefix?: string;
  /**
   * Operation log level.
   * The log is written to the standard output.
...
/**
 * Typescript transformer plugin for mangling identifiers.
 * !! The mangling is not fully safe, it can mangle only aaa.bbb accessor types. !!
 *
 * The transformer mangles:
 *   * private and protected class members
 *   * variables used in methods
 *
 * It can be used in two modes (separately, or both modes together in one compilation run).
 *   1. Mangle names for produced .js and .d.ts files
 *     * set the tsMangle for in before and afterDeclarations transformer definitions.
 *   2. Mangle names and store mangled .ts source a separate file in
 *      tsMangleOptions.mangledTsOutDir.
 *
 * Use with -noemit tsc option to produce just mangled file and neither .js, nor .d.ts.
 *
 * Transofmer API documentation:
 *   * https://github.com/itsdouges/typescript-transformer-handbook?tab=readme-ov-file
*/
export default function tsMangle(program: ts.Program, options: tsMangleOptions): ts.TransformerFactory<ts.SourceFile> {

  const mangleOptions = options ?? {};

    if (mangleOptions.loglevel == null)
      mangleOptions.loglevel = 0;

    //const exportsSymbolTree = new ExportsSymbolTree(program, []);

    const nameGen = new StringIdGenerator();

    function getNewMangledName(originalName: string): string {
      const prefix = mangleOptions.manglePrefix;
      if (prefix && prefix.length > 0) {
        //return "" + prefix + originalName;
        return nameGen.next() + "_" + prefix + originalName;
      }
      return nameGen.next();
    }
  ...
  return (context: ts.TransformationContext) => {
    ...
    function getNewName(originalName: string, type: VisibilityType): string {
      ...
      mangled = getNewMangledName(originalName);
    ...
    function transformNode(node: ts.Node): ts.Node {
      ...
      // 'node' in obj
      if (ts.isStringLiteral(node) && node.parent && ts.isBinaryExpression(node.parent) &&
      ...
      // obj['fooBar']
      if (ts.isStringLiteral(node) && node.parent && node.parent.kind
    ...
    function isTypePropertyExternal(type: ts.Type, typePropertyName: string): boolean {
      ...
      if (type.flags & ts.TypeFlags.Object) {
        ...
        // perhaps it would be awesome to handle exactly property we have, but Â¯\_(ãƒ„)_/Â¯
    ...
    function getSymbolVisibilityType(nodeSymbol: ts.Symbol): VisibilityType {
      ...
      if (isProtectedClassMember(nodeSymbol)) {
        return putToCache(nodeSymbol, VisibilityType.Private);
      }

      if (isPublicClassMember(nodeSymbol)) {
        return putToCache(nodeSymbol, VisibilityType.External);
      }
    ...
    // Normalize path and remove ./ and ../ from the path
    function sanitizePath(inputPath: string): string {
      return path.normalize(inputPath).replace(/^[\\\/\.]+/, "");
    }

    // Save transformed file into a separate file
    const saveTransformedFile = (sourceFile: ts.SourceFile, transformed: ts.SourceFile) => {
      if (transformed.fileName && mangleOptions.mangledTsOutDir && mangleOptions.mangledTsOutDir.length > 0) {

        //var fileName = path.basename(transformed.fileName);
        var relPath = path.relative(program.getCurrentDirectory(), transformed.fileName);
        relPath = sanitizePath(relPath);
        var outPath = path.join(mangleOptions.mangledTsOutDir, relPath);

        mangleOptions.loglevel && console.log(`Mangled .ts: ${relPath} -> ${outPath}`);

        fs.mkdirSync(path.dirname(outPath), { recursive: true }, (err) => {
          if (err) throw err;
        });

        const printer = ts.createPrinter();
        var printed = printer.printFile(transformed);
    ...
    return (sourceFile: ts.SourceFile) => {
      const transformed = transformNodeAndChildren(sourceFile, context);// as ts.SourceFile;
      saveTransformedFile(sourceFile, transformed);
      return transformed;
    };
  };
}
...
/*
tsc plugin interface (intention was to use just this transforem, save file and exit)
- just incomplete stub for tests
- probably not loaded by tsc in command line mode

typescript({
  compilerOptions: {
    plugins: [
      {
        name: "./tools/ts-mangle/ts-mangle-plugin.js",
        //type: "program",
        //transform: manglePlugin(),
      },
    ],
  },
    ...
    return (sourceFile: ts.SourceFile) => {
      ...
      const updatedStatements = [
        ts.factory.createExpressionStatement(
            ts.factory.createStringLiteral("// Transformed by customTransformer")
        ),
...
export function manglePlugin(info: ts.server.PluginCreateInfo) {
  return {
    ...
    emitFile: (fileName: string) => {
      ...
      const printer = ts.createPrinter();
      const transformed = ts.transform(sourceFile, [customTransformer()]);
      const printed = printer.printFile(transformed.transformed[0]);

      // Uložit transformovaný kód do souboru
      require("fs").writeFileSync(fileName.replace(".ts", ".transformed.ts"), printed);
    },
  };
}
